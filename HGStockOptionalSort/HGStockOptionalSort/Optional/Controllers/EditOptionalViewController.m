...
- (void)configureNavi {
    ...
    backButton.frame = CGRectMake(0, 0, 18, 15);
...
- (void)setUI {
    ...
    //创建tableView
    self.tableView = ({ UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, topViewHeight, VIEW_WIDTH, VIEW_HEIGHT - topViewHeight - bottomViewHeight - HGDeviceModelHelper.safeAreaInsetsTop - 44 - HGDeviceModelHelper.safeAreaInsetsBottom) style:UITableViewStylePlain];
    ...
    self.bottomView.frame = CGRectMake(0, VIEW_HEIGHT - bottomViewHeight - HGDeviceModelHelper.safeAreaInsetsTop - 44 - HGDeviceModelHelper.safeAreaInsetsBottom, VIEW_WIDTH, bottomViewHeight);
    ...
    [self.bottomView addSubview:deteleImageView];
...
- (void)deleteAction {
    ...
    [self.optionalList removeObjectsAtIndexes:indexes];
    //3.删除界面
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    weakcell.buttonClickBlock = ^(OptionalEditCell *cell) {
        ...
        id object = [self.optionalList objectAtIndex:fromIndexPath.row];
        [self.optionalList removeObjectAtIndex:fromIndexPath.row];
        [self.optionalList insertObject:object atIndex:toIndexPath.row];
    };
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.deleteQueue.count == 0) {
        //2.改变底部视图的颜色
        self.bottomView.backgroundColor = NAVIGATION_COLOR;
        //3.打开删除按钮的交互
