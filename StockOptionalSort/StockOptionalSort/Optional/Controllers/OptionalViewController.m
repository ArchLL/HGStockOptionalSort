...
#define leftTableViewWidth 90
#define rightUnitViewWidth 100   //rightTableView cell中每一项的宽度
#define optionalListTableName    @"OptionalOrder"  //自选排序表名
...
- (OptionalTopView *)optionalTopView {
    if (!_optionalTopView) {
        _optionalTopView = [[OptionalTopView alloc] initWithFrame:CGRectMake(0, 64, VIEW_WIDTH, topViewHeight) withTitles:self.dataTitles];
    }
...
- (void)setUI {
    ...
    weakSelf.optionalTopView.buttonClickBlock = ^(NSInteger tag) {
        if (tag == 1000) {
            ...
            editVC.optionalList = weakSelf.dataList;
            editVC.db = weakSelf.db;
            ...
            //右边菜单按钮的响应-排序
            //1.获取对应button
            UIButton *menuBtn = [weakSelf.optionalTopView viewWithTag:tag];
            //2.指定排序字段
            _orderBy = self.orderFields[tag-100];
            //3.指定排序方式
            if (menuBtn.selected) {
                //降序
                _upOrDown = @"-1";
    ...
    weakSelf.optionalTopView.scrollViewBlock = ^(CGPoint offset) {
        weakSelf.rightScrollView.contentOffset = offset;
    };
    //创建左边tableView
...
- (void)requestStockData {
    ...
    if (!kArrayIsEmpty(temArr)) {
        ...
        if (kStringIsEmpty(_orderBy)) {
            ...
            if ([_upOrDown isEqualToString:@"1"]) { //升序
                self.dataList = [self getOrderArrayWithArray:_dataList OrderBy:_orderBy IsAscend:YES];
            }else if([_upOrDown isEqualToString:@"-1"]){ //降序
                self.dataList = [self getOrderArrayWithArray:_dataList OrderBy:_orderBy IsAscend:NO];
            }
        }
    }
...
- (void)createTableView {
    //创建左边边tableView
    self.leftTableView = ({ UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 64 + topViewHeight, VIEW_WIDTH, VIEW_HEIGHT-64-49-topViewHeight) style:UITableViewStylePlain];
    ...
    _rightScrollView.contentSize = CGSizeMake(rightUnitViewWidth * self.dataTitles.count, 0);
    _rightScrollView.showsHorizontalScrollIndicator = NO;
    _rightScrollView.delegate = self;
    _rightScrollView.frame = CGRectMake(leftTableViewWidth, 64 + topViewHeight, VIEW_WIDTH-leftTableViewWidth-5, VIEW_HEIGHT-64-49-topViewHeight);
    [self.view addSubview:_rightScrollView];
    //创建righttableView
    self.rightTableView = ({ UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, _rightScrollView.contentSize.width, _rightScrollView.frame.size.height) style:UITableViewStylePlain];
        tableView.dataSource = self;
        tableView.delegate = self;
        tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == _leftTableView) {
        ...
        if (!cell) {
            cell = [[OptionalRightListCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"OptionalRightListCell" withUnitCount:_dataTitles.count];
        }
        if (self.dataList.count > 0) {
            cell.model = _dataList[indexPath.row];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    OptionalMarketModel *model = _dataList[indexPath.row];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView == _rightScrollView) {
        //左右滑动scrollView
